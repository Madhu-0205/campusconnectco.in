...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes: Array<{ route: string; priority: number; changeFreq: MetadataRoute.Sitemap[0]['changeFrequency'] }> = [
    // Core discovery & marketplace
    { route: '', priority: 1.0, changeFreq: 'daily' },
    { route: '/opportunities', priority: 0.95, changeFreq: 'daily' },
    { route: '/marketplace', priority: 0.9, changeFreq: 'daily' },
    { route: '/about', priority: 0.9, changeFreq: 'weekly' },
    { route: '/success-stories', priority: 0.9, changeFreq: 'weekly' },
  ...
  // 2. Fetch Open Gigs & Internships dynamically (strictly non-deleted)
  let gigUrls: MetadataRoute.Sitemap = [];
  let internshipUrls: MetadataRoute.Sitemap = [];
  let cityDiscoveryUrls: MetadataRoute.Sitemap = [];
  let cityGigUrls: MetadataRoute.Sitemap = [];
  let cityInternshipUrls: MetadataRoute.Sitemap = [];

  try {
    const activeFilter = getActiveOpportunityPrismaFilter();
    const [openGigs, openInternships] = await Promise.all([
      prisma.gig.findMany({
        where: {
    ...
    internshipUrls = openInternships.map((int: any) => ({
      url: `${baseUrl}/internships/${int.id}`,
      lastModified: int.updatedAt,
      changeFrequency: 'daily' as const,
      priority: 0.8,
    }));

    // Data-driven cities: only cities with at least 1 verified, active opportunity
    const activeGigCities = Array.from(
      new Set(
        openGigs
          .map(g => g.city?.toLowerCase().trim())
          .filter((c): c is string => Boolean(c))
      )
    );

    const activeInternshipCities = Array.from(
      new Set(
        openInternships
    ...
    const allActiveCities = Array.from(
      new Set([...activeGigCities, ...activeInternshipCities])
    );

    cityDiscoveryUrls = allActiveCities.map(city => ({
      url: `${baseUrl}/opportunities/${city}`,
      lastModified: new Date(),
      changeFrequency: 'daily' as const,
      priority: 0.8,
    }));

    cityGigUrls = activeGigCities.map(city => ({
    ...
    cityInternshipUrls = activeInternshipCities.map(city => ({
    ...
    console.error('Failed to query opportunities for sitemap:', error);
  }
  ...
  // 5. SEO landing pages for high-value, informational entry points
  ...
  // 6. Public Student Profile Pages (verified students with a username)
