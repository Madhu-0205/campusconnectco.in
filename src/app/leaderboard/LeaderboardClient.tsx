...
export function LeaderboardClient({ nonce }: { nonce?: string }) {
 ...
 <div className="absolute bottom-1/3 right-1/4 w-72 h-72 blur-[120px] rounded-full" style={{ background:"rgba(31,169,113,0.07)" }} />
 </div>
...
"url":"https://www.campusconnectco.in/leaderboard",
"description":"Top-ranked Indian students by verified gig completions and career achievement on CampusConnectCo.",
 ...
 style={period === p.value ? { background:"linear-gradient(135deg, #1FA971, #FF4500)" } : {}}
 >
 {p.label}
 </button>
 ))}
 ...
 style={{ background:"rgba(31,169,113,0.12)", border:"1px solid rgba(31,169,113,0.25)" }}
 >
 ...
 <div
 key={entry.userId}
 className={`flex items-center gap-3 p-4 rounded-2xl transition-all hover:border-[#1FA971]/30 ${entry.isCurrentUser ?"ring-1 ring-[#1FA971]/40" :""}`}
 style={{ background:"var(--color-surface)", border: `1px solid ${entry.isCurrentUser ?"rgba(31,169,113,0.3)" :"var(--color-border)"}` }}
 >
 <span className="text-sm font-black text-muted-foreground w-7 text-center shrink-0">#{entry.rank}</span>

 <div className="w-9 h-9 rounded-xl bg-[#1FA971]/15 flex items-center justify-center text-sm font-black text-[#A78BFA] shrink-0 relative overflow-hidden">
 {entry.avatar
 ...
 style={{ background:"linear-gradient(135deg, rgba(31,169,113,0.12), rgba(255,69,0,0.08))", border:"1px solid rgba(31,169,113,0.2)" }}
 >
 <Zap size={20} className="text-[#1FA971] mx-auto mb-3" />
