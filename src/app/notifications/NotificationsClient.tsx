...
export default function NotificationsClient({ initialNotifications = [] }: { initialNotifications?: NotificationItem[] }) {
 ...
 {unreadCount > 0 && (
 <span className="px-2.5 py-0.5 bg-[#1FA971] text-xs font-black rounded-full">
 {unreadCount} new
 </span>
 )}
 </div>
 ...
 className={`flex items-center gap-1.5 px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-all ${ activeTab === tab.key ?"bg-[#1FA971] text-white shadow-[0_0_20px_rgba(31,169,113,0.3)]" :"text-slate-400 hover:text-white" }`}
 >
 <Icon size={14} />
 {tab.label}
 {tabCount > 0 && (
 <span className={`px-1.5 py-0.5 rounded-full font-black ${ activeTab === tab.key ?"bg-white/30" :"bg-[#1FA971]/20 text-(--primary-light)" }`}>
 {tabCount}
 </span>
 )}
 </button>
 ...
 {/* Unread dot */}
 {!notif.read && (
 <div className="absolute top-4 left-1.5 w-1.5 h-1.5 rounded-full bg-[#1FA971]" />
 )}
