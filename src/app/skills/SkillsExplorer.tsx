...
const SKILLS: Skill[] = [
    ...
    { id: "ux-004", name: "ARIA Labels", category: "Accessibility", icon: "♿", color: "#7C3AED", keywords: ["aria-label", "icon-button", "screen-reader", "semantic-html", "accessibility"] },
    ...
    { id: "ux-035", name: "Style Matching", category: "UI Design", icon: "🎨", color: "#7C3AED", keywords: ["style-match", "product-type", "saas", "fintech", "glassmorphism", "neumorphism"] },
...
const CATEGORY_META: Record<string, { bg: string; border: string; text: string }> = {
    Accessibility: { bg: "#EDE9FE", border: "#7C3AED", text: "#5B21B6" },
    ...
    "UX Research": { bg: "#F5F3FF", border: "#7C3AED", text: "#4C1D95" },
};
        ...
        >
            ...
            <div className="flex items-start justify-between gap-2 pt-1">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="font-semibold text-slate-800 dark:text-slate-100 leading-tight">{skill.name}</p>
                        <span
                ...
                <button
                    ...
                    title="Copy skill ID"
                    className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center h-7 w-7 rounded-lg bg-white/5 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500"
                >
                    {copied ? (
            ...
            <div className="flex flex-wrap gap-1.5">
                {skill.keywords.slice(0, 5).map((kw) => (
                    <span
                        key={kw}
                        className="rounded-md bg-white/5 px-2 py-0.5 font-mono text-slate-500 dark:text-slate-400"
                    >
                        {kw}
                    </span>
                ))}
...
export default function SkillsExplorer() {
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-indigo-50/40 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950/30 px-4 py-12">
            <div className="mx-auto max-w-7xl">

                {/* ── Header ── */}
                <div className="mb-10 text-center">
                    <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/60 px-4 py-1.5 font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">
                        <span>⚡</span> UI/UX Pro Max
                    </div>
                    <h1 className="font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-5xl">
                        Skills Explorer
                    </h1>
                ...
                <div className="relative mb-6 mx-auto max-w-lg">
                    ...
                    <input
                        ...
                        className="w-full rounded-xl border border-white/10 bg-[#111116] py-3 pl-11 pr-4 text-slate-800 dark:text-slate-200 shadow-sm placeholder-slate-400 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/30 transition"
                    />
                    {query && (
                        <button
                            onClick={() => setQuery("")}
                            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 hover:bg-white/5 dark:hover:bg-slate-800 text-slate-400"
                        >
                    ...
                    {ALL_CATEGORIES.map((cat) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-1.5 rounded-xl px-3.5 py-1.5 font-semibold transition-all duration-200 border ${isActive ? "shadow-md scale-105" : "bg-[#111116] text-slate-500 dark:text-slate-400 border-white/10 hover:border-slate-300" }`}
                                style={
                                    isActive
                                        ? {
                ...
                {query || activeCategory !== "All" ? (
                    <p className="mb-5 dark:text-slate-500 text-center">
                        Showing <span className="font-semibold text-slate-600 dark:text-slate-300">{filtered.length}</span> of {SKILLS.length} skills
                    </p>
                ) : null}
                ...
                {filtered.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                        ...
                        <p className="dark:text-slate-400 font-medium text-lg">No skills match &ldquo;{query}&rdquo;</p>
                        <p className="text-slate-400">Try a different keyword or category</p>
                        <button
                            onClick={() => { setQuery(""); setActiveCategory("All"); }}
                            className="mt-2 rounded-xl bg-indigo-600 px-5 py-2 font-semibold text-white hover:bg-indigo-700 transition"
                        >
                            Clear filters
                        </button>
                    </div>
                ...
                {/* ── Footer ── */}
                <p className="mt-12 text-slate-400 dark:text-slate-600">
