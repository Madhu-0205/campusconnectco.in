...
export default function ClientApplicantsPage() {
 ...
 <div className="text-center py-20 bg-white/2 rounded-3xl border border-white/10">
 ...
 >
 <Card className={`p-6 rounded-3xl overflow-hidden transition-all ${application.status === 'ACCEPTED' ? 'bg-emerald-50/50 border-emerald-200 shadow-sm' : application.status === 'REJECTED' ? 'opacity-60 grayscale-50' : 'hover:shadow-lg bg-white'}`}>
 ...
 <span className="bg-blue-100 text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider flex items-center gap-1">
 <div className="w-1 h-1 rounded-full bg-blue-500 animate-pulse" /> New
 </span>
 )}
 {application.status === 'ACCEPTED' && (
 <span className="bg-emerald-100 text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider flex items-center gap-1 border border-emerald-200">
 <Check size={10} /> Hired
 </span>
 )}
 {application.status === 'REJECTED' && (
 <span className="bg-(--surface-2) text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider">
 Declined
 </span>
 )}
 </div>
 ...
 <p className="font-medium text-slate-500 bg-(--surface-2) p-3 rounded-xl border border-white/5 mb-3 italic">
 ...
 <span className="flex items-center gap-1 text-slate-600"><Sparkles size={12} className="text-(--accent)" /> {skillsStr}</span>
 ...
 className="w-full h-10 rounded-xl font-bold border-white/10 hover:border-(--primary)/40 hover:bg-white/2 :bg-surface-3 transition-all gap-2 shrink-0"
 >
 ...
 className="h-10 px-3 rounded-xl border-white/10 text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 :border-red-900/50 :bg-red-900/20 transition-all shrink-0"
 >
 <X size={18} />
 </Button>
 <Button
