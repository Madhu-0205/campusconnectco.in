...
export default async function CompanySEOPage({ params }: Props) {
 ...
 Campus Hiring & Internships at <span className="text-transparent bg-clip-text bg-linear-to-r from-primary-light to-primary-light">{comp.name}</span>
 </h1>
 ...
 <span className="flex items-center gap-1.5 font-medium text-slate-300">
 <MapPin size={13} className="text-primary" /> {comp.headquarters}
 </span>
 </div>
 ...
 <div
 key={internship.id}
 className="bg-surface/60 border border-white/5 rounded-3xl p-5 hover:border-cyan-500/20 transition-all flex flex-col justify-between h-52"
 >
 <div>
 ...
 <Link
 href="/auth/sign-up"
 className="inline-block px-4 py-2 rounded-xl bg-primary hover:bg-primary text-white font-bold text-xs transition-all"
 >
 Join as Student
 </Link>
 </div>
 ...
 return (
 <div key={student.id} className="bg-surface/80 border border-white/5 rounded-2xl p-4 space-y-3 hover:border-primary/20 transition-all">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-lg bg-linear-to-br from-primary-light to-primary-light flex items-center justify-center font-black text-xs text-white">
 {avatarLetter}
 </div>
 ...
 <Link href={`/profile/${student.username || student.id}`} className="inline-flex items-center gap-0.5 text-primary hover:text-primary font-bold">
 ...
 <div className="bg-gradient-to-br from-primary-light/10 to-primary-light/5 border border-cyan-500/10 rounded-3xl p-5 space-y-2.5">
