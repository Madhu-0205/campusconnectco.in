...
export default async function CityInternshipsPage({ params }: Props) {
 ...
 Student Internships in <span className="text-transparent bg-clip-text bg-linear-to-r from-primary to-primary">{cityName}</span>
 </h1>
 ...
 <Link href="/dashboard" className="px-5 py-2.5 rounded-xl bg-primary hover:bg-primary text-white font-bold text-sm transition-all">
 Go to Dashboard
 </Link>
 ...
 <div key={internship.id} className="bg-surface/60 border border-white/5 rounded-3xl p-5 hover:border-primary/20 transition-all flex flex-col justify-between h-52">
 <div>
 ...
 <span className="font-mono text-primary font-bold text-sm">{internship.stipend ? `₹${internship.stipend.toLocaleString("en-IN")}/mo` :"Unpaid"}</span>
