...
export default async function CityGigsPage({ params }: Props) {
 ...
 Student Gigs & Projects in <span className="text-transparent bg-clip-text bg-linear-to-r from-primary to-primary">{cityName}</span>
 </h1>
 ...
 <Link href="/post-gig" className="px-5 py-2.5 rounded-xl bg-primary hover:bg-primary text-white font-bold text-sm transition-all">
 ...
 {gigs.map((gig: any) => (
 <div key={gig.id} className="bg-surface/60 border border-white/5 rounded-3xl p-5 hover:border-primary/20 transition-all flex flex-col justify-between h-52">
 <div>
 ...
 <span className="font-mono text-primary font-bold text-sm">₹{gig.budget.toLocaleString("en-IN")}</span>
