    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <section>
        <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
          <Zap className="text-violet-400" /> AI Recommendation Engine Performance
        </h2>
          ...
          <div className="space-y-8">
            {funnel.map((step, idx) => (
              <div key={step.step as string} className="relative">
                {idx !== funnel.length - 1 && (
                  <div className="absolute left-6 top-10 w-0.5 h-16 bg-slate-800"></div>
                )}
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center font-bold text-slate-400 border border-slate-700 z-10">
                    {idx + 1}
                  </div>
                  <div className="flex-1 bg-slate-800/50 p-4 rounded-xl border border-slate-800/50 flex justify-between items-center">
                    <div>
