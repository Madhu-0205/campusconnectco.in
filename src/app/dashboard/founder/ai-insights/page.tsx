...
export default async function AIInsightsPage() {
    ...
    const [gigs, apps, escrows] = await Promise.all([
        prisma.gig.findMany({ select: { createdAt: true, title: true, status: true }, orderBy: { createdAt: "desc" } }),
        prisma.application.count({ where: { createdAt: { gte: lastMonth } } }),
        prisma.escrow.findMany({ select: { createdAt: true, amount: true, status: true } })
    ]);
