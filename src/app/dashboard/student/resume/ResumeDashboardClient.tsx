...
export default function ResumeDashboardClient() {
 ...
 <BrainCircuit className="text-primary" /> Resume Intelligence
 </h1>
 ...
 className={`w-full text-left p-4 rounded-xl border transition-all ${activeAnalysis?.id === a.id ? 'bg-primary/10 border-primary/50' : 'bg-(--surface-2) border-(--border) hover:border-white/20'}`}
 >
 ...
 <Radar name="ATS" dataKey="A" stroke="#1FA971" fill="#1FA971" fillOpacity={0.4} />
 </RadarChart>
 </ResponsiveContainer>
 </div>
 </div>
 ...
 <Sparkles className="w-5 h-5 text-primary" /> Suggested Improvements
 </h3>
 ...
 <span key={i} className="px-3 py-1 bg-primary/10 border border-primary/20 text-primary rounded-lg text-xs font-bold">
 {v}
 </span>
 ))}
 </div>
 ...
 {/* Career Roadmap */}
 <div className="bg-linear-to-br from-primary/20 to-primary/10 border border-primary/20 rounded-3xl p-8 relative overflow-hidden">
 ...
 className="shrink-0 px-6 py-3 bg-primary hover:bg-primary disabled:opacity-50 text-white font-bold rounded-xl transition-all flex items-center gap-2 shadow-[0_0_20px_rgba(31,169,113,0.3)]"
 >
 ...
 <div className="text-xs font-bold text-primary">{q.difficulty}</div>
