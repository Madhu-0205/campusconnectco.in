...
export default function InterviewSimulatorPage() {
 ...
 <History size={18} className="text-primary" /> Simulations
 </h2>
 {phase !=="setup" && (
 <Button variant="ghost" size="icon" onClick={returnToSetup} className="text-primary hover:text-primary hover:bg-primary/20 rounded-full h-8 w-8">
 <Plus size={18} />
 </Button>
 )}
 </div>
 ...
 className={`w-full text-left p-4 rounded-2xl border transition-all group ${activeInterview?.id === item.id ? 'bg-primary/10 border-primary/40' : 'bg-[#0a0a0f] border-white/5 hover:border-primary/30'}`}
 >
 <div className="flex justify-between items-start mb-1">
 <p className="font-bold text-white line-clamp-1 group-hover:text-primary transition-colors text-sm">
 {item.roleTitle}
 </p>
 {item.score !== null && (
 ...
 <div className="w-16 h-16 rounded-3xl bg-primary/20 border border-primary/30 flex items-center justify-center text-primary shadow-2xl">
 <Brain size={32} />
 </div>
 ...
 <Shield size={14} className="text-primary" /> Target Engineering Role
 </label>
 ...
 className="w-full h-14 px-4 bg-[#0a0a0f] border border-white/10 rounded-2xl text-white font-bold focus:border-primary focus:outline-none transition-colors"
 >
 {DEFAULT_ROLES.map(r => (
 ...
 <div className="space-y-3">
 <label className="font-black uppercase tracking-widest text-slate-400 text-xs flex items-center gap-2">
 <TrendingUp size={14} className="text-primary" /> Difficulty Level
 </label>
 ...
 className={`flex-1 h-14 rounded-2xl border font-black text-xs transition-all active:scale-95 ${difficulty === lvl ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-transparent text-slate-400 border-white/10 hover:border-white/20'}`}
 >
 {lvl}
 </button>
 ))}
 ...
 <Badge className="bg-primary/20 text-primary border border-primary/30 font-black px-3 py-1">
 LIVE INTERVIEW
 </Badge>
 </div>
 ...
 <div className={`w-8 h-8 rounded-xl shrink-0 flex items-center justify-center font-black text-xs ${msg.role ==="user" ?"bg-primary/20 border border-primary/30 text-primary" :"bg-linear-to-br from-primary to-primary-light text-white"}`}>
 {msg.role ==="user" ?"U" :"AI"}
 </div>
 <div className={`p-4 rounded-2xl max-w-[80%] text-sm leading-relaxed ${msg.role ==="user" ?"bg-primary/15 border border-primary/20 text-slate-200" :"bg-[#0a0a0f] border border-white/5 text-slate-300"}`}>
 ...
 className="bg-primary hover:bg-primary text-white font-black px-6 h-11 rounded-xl flex items-center gap-2 shadow-lg active:scale-95 transition-all"
 >
 {isSubmitting ? (
 ...
 { label:"Structure, Logic & Systematic Approach", value: activeInterview.feedback?.structure || 70, color:"bg-primary" }
