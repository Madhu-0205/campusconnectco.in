    ...
    <div className="bg-[#111127]/60 border border-white/5 rounded-3xl p-6 lg:p-8">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-white mb-2">
          Your Path to <span className="text-violet-400 capitalize">{roadmap.targetCareer}</span>
        </h2>
          ...
          return (
            <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#08080F] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow transition-colors ${
                isCompleted ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-500'
              }`}>
                {isCompleted ? <CheckCircle2 size={18} /> : getStepIcon(step.type)}
              </div>
              
              {/* Content card */}
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl bg-black/40 border border-white/5 hover:border-violet-500/20 transition-colors">
                <div className="flex items-center justify-between mb-2">
                  <h3 className={`font-bold text-lg ${isCompleted ? 'text-slate-300' : 'text-white'}`}>
                    {step.title}
                  </h3>
                  {step.actionableLink && !isCompleted && (
                    <Link href={step.actionableLink} className="text-xs text-cyan-400 font-bold uppercase tracking-wider flex items-center gap-1 hover:text-cyan-300">
