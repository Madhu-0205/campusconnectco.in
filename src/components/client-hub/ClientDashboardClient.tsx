...
export function KanbanBoard({ recentGigs }: KanbanProps) {
 ...
 <button className="h-10 px-5 rounded-xl bg-[#1FA971] hover:bg-[#6D28D9] font-bold text-sm transition-all shadow-lg shadow-primary/20 active:scale-95">
 Post First Gig
 </button>
 </Link>
 </motion.div>
 ...
 className="block bg-surface border border-white/5 hover:border-primary/30 rounded-xl p-4 transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.4)] group relative overflow-hidden active:scale-[0.98]"
 >
 <div className="absolute inset-0 bg-linear-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
 <p className="font-bold text-white group-hover:text-primary transition-colors line-clamp-2 mb-3 relative z-10">
 {gig.title}
 </p>
...
export function TopApplicants({ applicants }: { applicants: ApplicantRecord[] }) {
 ...
 >
 <div className="w-11 h-11 rounded-xl bg-linear-to-br from-primary to-sky-500 flex items-center justify-center font-black shrink-0 text-sm shadow-lg shadow-primary/10 transition-transform group-hover:scale-105">
 {(app.applicant?.name ||"?").slice(0, 2).toUpperCase()}
 </div>
 <div className="flex-1 min-w-0">
 <p className="font-bold text-sm truncate group-hover:text-primary transition-colors">{app.applicant?.name ||"Anonymous"}</p>
 ...
 className="flex items-center gap-1 font-bold px-3 py-1.5 bg-primary text-white hover:bg-primary rounded-lg transition-all shadow-md shadow-primary/20 active:scale-95"
 >
 Review
 </Link>
 </div>
