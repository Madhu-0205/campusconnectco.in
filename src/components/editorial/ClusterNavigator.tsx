        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {cluster.associatedRoles.map(role => (
              <Link 
                key={role}
                href={`/careers/${role}`}
                className="px-3 py-1.5 rounded-lg bg-black/40 border border-white/5 text-xs text-slate-300 hover:text-white hover:border-violet-500/30 transition-all"
              >
          ...
          <div className="pt-4 border-t border-white/5">
            <Link 
              ...
              className="inline-flex items-center gap-2 text-sm font-bold text-violet-400 hover:text-cyan-400 transition-colors"
            >
