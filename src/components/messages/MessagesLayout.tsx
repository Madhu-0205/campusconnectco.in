...
export function MessagesLayout({ initialConversations, currentUserId, initialActiveId }: MessagesLayoutProps) {
  ...
  if (!mounted) {
    return (
      <div className="flex h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)] bg-background border border-white/5 rounded-5xl overflow-hidden animate-pulse">
        <aside className="hidden md:flex w-96 bg-card border-white/5 flex-col p-8 gap-6">
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] md:h-[calc(100vh-8rem)] bg-background border border-white/5 rounded-5xl overflow-hidden shadow-3xl">
      ...
      <aside className={cn(
        "w-full md:w-96 bg-card border-r border-white/5 flex flex-col md:flex",
        activeId && "hidden md:flex"
      )}>
        <div className="p-8 space-y-6">
          <div className="flex items-center justify-between">
             <h2 className="font-black text-foreground tracking-tight">Messages</h2>
             <button className="w-10 h-10 rounded-2xl bg-(--surface-2) border border-border flex items-center justify-center text-foreground hover:text-foreground transition-all">
                <Plus size={20} />
             </button>
          </div>
          ...
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full bg-white/2 border border-border rounded-2xl py-3 pl-12 pr-4 font-bold text-foreground placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-foreground/50 transition-all"
            />
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "bg-foreground text-background shadow-xl shadow-indigo-600/20" : "hover:bg-(--surface-2)"
                )}
              >
                <div className="relative shrink-0">
                  <div className="w-14 h-14 rounded-2xl overflow-hidden bg-card border-white/5 group-hover:scale-105 transition-transform">
                    {(r.avatar_url || r.image) ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center font-black text-foreground">
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className={cn("text-sm font-black truncate", isActive ? "text-foreground" : "text-foreground")}>
                      {r.full_name || r.name}
                    </p>
                    <span className={cn("text-[9px] font-black uppercase tracking-tight", isActive ? "text-foreground/60" : "text-muted-foreground")}>
                      {conv.last_message_at ? format(new Date(conv.last_message_at), 'H:mm') : ''}
                    </span>
                  </div>
                  <p className={cn("text-xs truncate font-medium", isActive ? "text-foreground/80" : "text-muted-foreground")}>
      ...
      <main className={cn(
        ...
        {activeConversation ? (
          <>
            {/* Header */}
            <header className="h-24 px-8 border-white/5 flex items-center justify-between bg-card/50 backdrop-blur-3xl">
              <div className="flex items-center gap-4">
                <button onClick={() => setActiveId(null)} className="md:hidden p-2 text-muted-foreground hover:text-foreground">
                  <ArrowLeft size={20} />
                </button>
                <div className="relative">
                  <div className="w-12 h-12 rounded-2xl overflow-hidden bg-card border border-border">
              ...
              <div className="flex items-center gap-2">
                 <button className="p-3 rounded-2xl bg-(--surface-2) border border-white/5 text-muted-foreground hover:text-foreground transition-all"><Phone size={18} /></button>
                 <button className="p-3 rounded-2xl bg-(--surface-2) border border-white/5 text-muted-foreground hover:text-foreground transition-all"><Video size={18} /></button>
                 <button className="p-3 rounded-2xl bg-(--surface-2) border border-white/5 text-muted-foreground hover:text-foreground transition-all"><MoreVertical size={18} /></button>
              </div>
            </header>
                  ...
                  return (
                    ...
                    >
                      <div className={cn(
                        ...
                        isMe 
                          ? "bg-foreground text-background text-foreground rounded-tr-none shadow-indigo-500/10" 
            ...
            {/* Input Area */}
            <div className="p-6 md:p-8 bg-card/80 backdrop-blur-3xl border-white/5">
              <form onSubmit={handleSendMessage} className="relative group">
                <div className="absolute inset-0 bg-indigo-500/5 blur-2xl rounded-full group-focus-within:bg-accent transition-all" />
                <div className="relative flex items-center bg-[#18181F] border border-white/5 group-focus-within:border-indigo-500/30 rounded-3xl p-1.5 shadow-2xl transition-all">
                  ...
                  <div className="flex items-center gap-1">
                    <button type="button" className="p-3 text-muted-foreground hover:text-foreground transition-colors hidden sm:block">
                      <Smile size={20} />
                    </button>
                    <button 
                      type="submit"
                      className="bg-foreground text-background hover:bg-foreground/90 text-foreground p-4 rounded-2xl shadow-xl shadow-indigo-600/20 active:scale-95 transition-all"
                    >
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
             <div className="relative mb-8 group">
                ...
                <div className="relative w-24 h-24 rounded-4xl bg-card border border-border flex items-center justify-center shadow-2xl">
                   <MessageSquare size={40} className="text-foreground" />
                </div>
             </div>
             <h3 className="font-black text-foreground tracking-tight mb-4">Select a Conversation</h3>
             ...
             <div className="mt-10 flex gap-4">
                <div className="flex items-center gap-2 px-4 py-2 bg-(--surface-2) border border-border rounded-xl font-black text-muted-foreground uppercase tracking-widest">
