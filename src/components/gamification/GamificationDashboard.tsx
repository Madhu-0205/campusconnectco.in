...
function XpBar({ progress, levelTitle, level, nextLevelXp, totalXp }: {
 ...
 <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full blur-3xl" style={{ background:"rgba(31,169,113,0.15)" }} />
 ...
 <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider text-[#A78BFA] bg-[#1FA971]/15 border border-[#1FA971]/25">
 {levelTitle}
 </span>
 </div>
 <p className="text-xs text-muted-foreground mt-0.5">{totalXp.toLocaleString()} / {nextLevelXp.toLocaleString()} XP</p>
 </div>
 <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-black" style={{ background:"linear-gradient(135deg, #1FA971, #FF4500)" }}>
 ...
 style={{ width: `${progress}%`, background:"linear-gradient(90deg, #1FA971, #FF4500)", boxShadow:"0 0 12px rgba(31,169,113,0.5)" }}
 />
 </div>
...
function ScoreRadar({ reliability, execution, learning, community, smart }: {
 ...
 { label:"Learning", value: learning, icon: Brain, color:"#1FA971", desc:"Skills & assessments" },
...
function XpFeed({ events }: { events: GamificationProfile["xpEvents"] }) {
 ...
 <h3 className="font-black text-foreground text-sm mb-4 flex items-center gap-2">
 <Zap size={13} className="text-[#1FA971]" />
 Recent XP
 </h3>
...
export function GamificationDashboard() {
 ...
 { label:"Smart Score", value: profile.smartScore, icon: Star, color:"text-[#1FA971]" },
