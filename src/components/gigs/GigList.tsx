...
export async function GigList({ searchParams }: { searchParams?: { q?: string; lat?: string; lng?: string; category?: string } }) {
 ...
 return (
 <div className="p-8 text-center rounded-2xl border border-red-200 bg-red-50">
 <p className="font-black text-red-700 mb-1">Failed to load gigs</p>
 ...
 <h3 className="font-black text-muted-foreground mb-1">No gigs found</h3>
 ...
 <span className="text-foreground font-black">{gigs.length}</span> opportunities found
 </p>

 {ratedGigs.map((gig) => (
 <div key={gig.id}
 className="group relative bg-white border border-border rounded-2xl p-6 hover:border-electric/40 hover:shadow-lg transition-all overflow-hidden cursor-pointer">
 ...
 {/* Avatar */}
 <div className="w-14 h-14 rounded-2xl bg-accent shrink-0 flex items-center justify-center font-black group-hover:bg-accent text-foreground group-hover:text-foreground transition-colors border border-border">
 ...
 <h3 className="font-black text-foreground group-hover:text-foreground transition-colors truncate">
 {gig.title}
 </h3>
 {gig.matchScore > 0 && (
 <span className={`font-black px-2 py-0.5 rounded-full shrink-0 ${gig.matchScore > 60 ?"bg-emerald-100" :"bg-accent text-muted-foreground"}`}>
 ...
 <span key={gs.skillId} className="px-2.5 py-1 rounded-lg font-bold bg-accent text-muted-foreground border border-border">
 {gs.skill.name}
 </span>
 ))}
 </div>
