...
export function FloatingGigCard({ isOpen, onClose, gig }: FloatingGigCardProps) {
 ...
 className={`relative w-full max-h-[90vh] overflow-hidden bg-white rounded-4xl shadow-2xl border border-border border-border flex flex-col ${isExpanded ?"max-w-4xl" :"max-w-sm sm:max-w-md lg:max-w-lg" }`}
 >
 {/* Close Button */}
 <button
 onClick={onClose}
 ...
 className={`p-2 rounded-full transition-all ${isSaved ?"bg-rose-50" :"bg-background text-muted-foreground hover:text-rose-500 bg-accent hover:bg-rose-50 :bg-rose-500/10"}`}
 >
 ...
 <motion.div
 layout
 className={`p-6 sm:p-8 bg-background/50 flex flex-col gap-6 justify-between ${isExpanded ?"md:w-2/5" :"w-full border-slate-100 border-border"}`}
 >
 {/* Packages Toggle */}
