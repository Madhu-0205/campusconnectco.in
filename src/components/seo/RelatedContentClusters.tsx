    ...
    <section className="mt-16 border-t border-white/10 pt-12 space-y-10" aria-label="Explore Related Student Opportunity Clusters">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {currentType !== "category" && (
          <div className="bg-[#111127]/60 border border-white/5 rounded-3xl p-5 space-y-3">
            <h3 className="text-sm font-bold text-violet-400 flex items-center gap-1.5 uppercase tracking-wider font-mono">
            ...
            <div className="flex flex-wrap gap-2">
              {INTERNSHIP_CATEGORIES.map((cat) => (
                <Link
                  ...
                  className="px-3 py-1.5 bg-white/5 hover:bg-violet-600/20 border border-white/5 hover:border-violet-500/30 rounded-xl text-xs text-slate-300 hover:text-white transition-all flex items-center gap-1"
                >
                  <span>{cat.name}</span>
        ...
        {currentType !== "skill" && (
          <div className="bg-[#111127]/60 border border-white/5 rounded-3xl p-5 space-y-3">
            <h3 className="text-sm font-bold text-indigo-400 flex items-center gap-1.5 uppercase tracking-wider font-mono">
            ...
            <div className="flex flex-wrap gap-2">
              {FEATURED_SKILLS.map((sk) => (
                <Link
                  key={sk.slug}
                  href={`/skills/${sk.slug}`}
                  className="px-3 py-1.5 bg-white/5 hover:bg-indigo-600/20 border border-white/5 hover:border-indigo-500/30 rounded-xl text-xs text-slate-300 hover:text-white transition-all flex items-center gap-1"
                >
                  <span>{sk.name}</span>
                  <ArrowUpRight size={11} className="text-slate-500" />
                </Link>
