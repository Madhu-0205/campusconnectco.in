...
function ReputationBadge({ score }: { score: number }) {
 ...
 score >= 60 ? { label:"Advanced", color:"text-[#1FA971]", bg:"bg-[#1FA971]/15 border-[#1FA971]/30" } :
...
export function TalentSearchClient({
 ...
 Filters
 {hasFilters && (
 <span className="w-4 h-4 bg-[#1FA971] rounded-full text-[9px] font-black text-white flex items-center justify-center">
 !
 </span>
 )}
 </button>
 ...
 selectedTier === t
 ?"bg-[#1FA971]/25 border-[#1FA971]/50 text-[#A78BFA]"
 ...
 border: `1px solid ${isLocked ?"var(--color-border)" :"rgba(31,169,113,0.2)"}`,
 }}
 >
 {/* Locked overlay */}
 {isLocked && (
 ...
 <button className="text-[10px] font-black px-3 py-1.5 rounded-lg bg-[#1FA971]/30 border border-[#1FA971]/50 text-[#A78BFA] hover:bg-[#1FA971]/50 transition-all">
 Unlock — ₹3,499/mo
 </button>
 </Link>
 </div>
 ...
 {/* Hover glow */}
 {!isLocked && (
 <div className="absolute -right-8 -top-8 w-24 h-24 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-[#1FA971]/20" />
 )}
 ...
 style={{ background:"linear-gradient(135deg, var(--color-primary), #1FA971)" }}
 >
 {initials}
 </div>
 )}
 ...
 <div className="text-lg font-black text-[#1FA971]">{candidate.matchScore}%</div>
 ...
 <button className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold border border-white/10 hover:border-[#1FA971]/40 hover:bg-[#1FA971]/10 text-slate-400 hover:text-[#A78BFA] transition-all">
 <Eye size={12} />
 View Profile
 </button>
 </Link>
