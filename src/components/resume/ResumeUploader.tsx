...
export function ResumeUploader({ onUploadComplete, currentFileUrl }: { onUploadComplete: (url: string) => void, currentFileUrl?: string }) {
 ...
 <button onClick={() => setFile(null)} className="text-xs font-bold px-4 py-2 bg-primary hover:bg-primary rounded-lg transition-colors" type="button" onClickCapture={() => document.getElementById("hidden-file-input")?.click()}>
 Replace
 </button>
 ...
 className="border-2 border-dashed border-white/20 hover:border-primary/50 rounded-2xl p-10 flex flex-col items-center justify-center text-center cursor-pointer transition-colors bg-(--surface) hover:bg-white/2"
 >
 ...
 <button 
 onClick={uploadFile} 
 className="flex-1 bg-primary hover:bg-primary text-white font-bold text-xs py-2.5 rounded-xl transition-all"
 >
