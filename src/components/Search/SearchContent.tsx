    ...
    return (
        <div className="min-h-screen bg-background pt-24 pb-12">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                {/* Search Header */}
                <div className="mb-8">
                    <h1 className="md:text-4xl font-black text-white mb-4">
                        Search Results
                    </h1>
                    ...
                    <form onSubmit={handleSearch} className="relative max-w-2xl">
                        ...
                        <input
                            ...
                            className="w-full bg-(--surface-2) border border-(--border) rounded-2xl pl-12 pr-4 py-4 text-white focus:ring-2 focus:ring-(--primary)/50 outline-none shadow-sm"
                            value={searchQuery}
                    ...
                    {results && !isLoading && (
                        <p className="mt-4 text-slate-400">
                            Found <strong className="text-(--primary)">{totalCount}</strong> results for &quot;{results.query}&quot;
                        </p>
                    )}
                </div>
                ...
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                    {[
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl font-semibold whitespace-nowrap transition-all ${activeTab === tab.key ? "bg-(--primary) shadow-lg" : "bg-(--surface-2) text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" }`}
                        >
                    ...
                    <div className="space-y-8">
                                                    ...
                                                    {gig.tags && (
                                                        <div className="flex flex-wrap gap-2 mt-3">
                                                            {gig.tags.split(",").slice(0, 3).map((tag, idx) => (
                                                                <span
                                                                    key={idx}
                                                                    className="px-2 py-1 bg-(--primary)/10 text-xs font-semibold rounded-lg"
                                                                >
                                                                    {tag.trim()}
                                                                </span>
                                                            ))}
                                    ...
                                    {results.results.users.map((user) => (
                                        <Card key={user.id} className="p-6 hover:shadow-lg transition-shadow">
                                            <div className="flex items-start gap-4">
                                                <div className="h-12 w-12 rounded-full bg-(--primary) text-white font-bold flex items-center justify-center shrink-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-slate-500 dark:text-slate-400 capitalize mb-2">
                                                    ...
                                                    {user.skills && (
                                                        <div className="flex flex-wrap gap-1 mt-2">
                                                            {user.skills.split(",").slice(0, 3).map((skill, idx) => (
                                                                <span
                                                                    key={idx}
                                                                    className="px-2 py-0.5 bg-white/5 text-xs rounded"
                                                                >
                                                                    {skill.trim()}
                                                                </span>
                                                            ))}
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {results.results.skills.map((skill, idx) => (
                                        <button
                                            ...
                                            className="px-4 py-2 bg-(--surface-2) border border-(--border) rounded-xl font-semibold text-slate-300 hover:border-(--primary) hover:text-(--primary) transition-all"
                                        >
                                            {skill}
                                        </button>
                                    ))}
                        ...
                        {totalCount === 0 && (
                            <div className="text-center py-20">
                                <Search size={64} className="mx-auto text-slate-300 dark:text-slate-700 mb-4" />
