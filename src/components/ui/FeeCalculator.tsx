...
export default function FeeCalculator({ defaultValue = 2000, className ="" }: FeeCalculatorProps) {
 ...
 return (
 <div className={`bg-[#0D1527]/80 border border-[#1FA971]/25 rounded-3xl p-8 backdrop-blur-sm shadow-[0_0_60px_rgba(31,169,113,0.10)] ${className}`}>
 ...
 style={{
 background: `linear-gradient(to right, #1FA971 ${sliderPct}%, rgba(255,255,255,0.1) ${sliderPct}%)`,
 }}
 />
 ...
 className={`flex-1 py-2.5 px-3 rounded-xl font-bold border transition-all ${ tierIdx === i ?"bg-[#1FA971] border-[#1FA971] shadow-[0_0_20px_rgba(31,169,113,0.4)]" :"border-white/10 text-slate-400 hover:border-white/20 hover:text-white" }`}
 >
 {t.label}
 </button>
 ))}
 ...
 { label:"Platform Earns", value: `₹${platformEarns.toLocaleString("en-IN")}`, color:"text-[#1FA971]", bg:"bg-[#1FA971]/10 border-[#1FA971]/20" },
 ].map((item) => (
 <motion.div
 key={item.label}
 layout
