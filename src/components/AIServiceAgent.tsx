...
const QUICK_ACTIONS: QuickAction[] = [
 { icon: Brain, label:"SmartMatch", path:"/dashboard/student/smartmatch", color:"text-primary" },
...
export default function AIServiceAgent() {
 ...
 className={`pointer-events-auto mb-4 flex flex-col rounded-2xl overflow-hidden border border-white/10 shadow-[0_24px_80px_rgba(0,0,0,0.6)] bg-surface backdrop-blur-xl transition-all duration-300 ${isMinimized ?"w-80" :"w-[calc(100vw-3rem)] sm:w-[380px] md:w-[400px] h-[70vh] min-h-100 max-h-[620px]" }`}
 >
 {/* Header */}
 <div
 className="h-[60px] shrink-0 flex items-center justify-between px-4"
 style={{
 background:"linear-gradient(135deg, #1FA971 0%, #5B21B6 50%, #0EA5E9 100%)",
 }}
 >
 <div className="flex items-center gap-3">
 <div className="relative w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center shadow-inner">
 <Sparkles size={16} className="text-white" />
 <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border-[var(--surface)]" />
 </div>
 <div>
 ...
 <>
 {showHistory ? (
 <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-surface custom-scrollbar">
 <div className="flex items-center justify-between mb-4">
 <h3 className="font-bold text-xs text-slate-300">Previous Conversations</h3>
 <button
 onClick={startNewSession}
 className="flex items-center gap-1 text-[10px] font-bold text-primary hover:text-primary bg-primary/10 px-2.5 py-1 rounded-lg border border-primary/20 active:scale-95 transition-all"
 >
 ...
 className={`group w-full text-left p-3.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${activeSessionId === s.id ? 'bg-primary/15 border-primary/40' : 'bg-surface border-white/5 hover:border-white/15 hover:bg-white/2'}`}
 >
 <div className="flex-1 min-w-0 pr-2">
 <p className="font-bold text-xs text-slate-200 truncate group-hover:text-primary transition-colors">
 ...
 <>
 {/* Quick Nav Pills */}
 <div className="shrink-0 flex gap-2 px-3 py-2.5 overflow-x-auto scrollbar-none border-white/5 bg-surface">
 ...
 >
 {/* Avatar */}
 <div className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center font-bold mt-0.5 ${msg.role ==="user" ?"bg-primary/30 border border-primary/20" :"bg-linear-to-br from-primary to-blue-600 text-white" }`}>
 {msg.role ==="user" ?"U" : <Sparkles size={12} />}
 </div>

 {/* Bubble */}
 <div className={`max-w-[82%] ${msg.role ==="user" ?"items-end" :"items-start"} flex flex-col`}>
 <div className={`px-3.5 py-2.5 leading-relaxed rounded-2xl ${msg.role ==="user" ?"bg-primary shadow-[0_0_20px_rgba(31,169,113,0.3)] text-white rounded-tr-sm" :"bg-surface text-slate-200 rounded-tl-sm border border-white/8 text-xs" }`}>
 ...
 className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 hover:bg-primary/20 border border-primary/25 text-[10px] font-medium transition-all hover:border-primary/40"
 >
 ...
 {/* Input */}
 <div className="shrink-0 p-3 bg-surface border-t border-white/5">
 <form
 onSubmit={(e) => { e.preventDefault(); sendMessage(); }}
 className="flex items-center gap-2 bg-surface border border-white/10 rounded-xl px-3 py-2 focus-within:border-primary/50 transition-colors"
 >
 <input
 ref={inputRef}
 type="text"
 ...
 className="w-8 h-8 flex items-center justify-center rounded-lg bg-primary hover:bg-primary transition-colors disabled:opacity-40 disabled:bg-white/5 shrink-0"
 >
 {isStreaming
 ...
 whileTap={{ scale: 0.93 }}
 onClick={handleOpen}
 className="pointer-events-auto relative w-14 h-14 rounded-full flex items-center justify-center shadow-[0_8px_32px_rgba(31,169,113,0.5)] hover:shadow-[0_12px_40px_rgba(31,169,113,0.7)] transition-shadow duration-300 border border-white/10"
 style={{
 background:"linear-gradient(135deg, #1FA971 0%, #5B21B6 50%, #0EA5E9 100%)",
 }}
 >
 ...
 {/* Live indicator */}
 <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-emerald-400 border-[var(--surface)] rounded-full">
 {showPulse && (
 ...
 {/* Tooltip */}
 <span className="absolute right-16 top-1/2 -translate-y-1/2 bg-surface border border-white/10 text-xs font-medium px-3 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none shadow-xl transition-opacity">
 AI Career Guide
 </span>
 </motion.button>
 )}
