...
function highlight(text: string, query: string) {
 ...
 <>
 {text.slice(0, idx)}
 <mark className="bg-yellow-200 rounded px-0.5 not-italic">
...
export function SkillBadge({ skill, onRemove, readOnly }: SkillBadgeProps) {
 ...
 className="ml-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full transition-colors duration-150 hover:bg-black/10 :bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1"
...
function DropdownOption({
 ...
 className={`flex cursor-pointer items-center gap-3 px-3 py-2.5 transition-colors duration-100 ${isActive ?"bg-slate-100" :"hover:bg-slate-50 :bg-surface-3/60" }`}
 >
 {/* Icon bubble */}
 <span
...
export default function SkillSelector({
 ...
 id={labelId}
 htmlFor={inputId}
 className="mb-1.5 block font-semibold text-slate-700"
 >
 {label}
 ...
 {/* Control box */}
 <div
 className={`relative flex min-h-[46px] cursor-text flex-wrap items-center gap-1.5 rounded-xl border bg-white px-3 py-2 transition-all duration-200 ${isOpen ?"border-primary ring-2 ring-primary/25" :"border-slate-200 hover:border-slate-300 :border-slate-600" } ${disabled ?"opacity-50 pointer-events-none" :""}`}
 ...
 autoComplete="off"
 spellCheck={false}
 className={`flex-1 min-w-[120px] bg-transparent text-slate-800 placeholder-slate-400 outline-none ${reachedMax ?"opacity-40 pointer-events-none" :""}`}
 onChange={(e) => {
 ...
 className="ml-1 shrink-0 rounded-full p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 :bg-surface-3 :text-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
 >
 ...
 {isOpen && !disabled && (
 <div
 className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50 animate-[dropIn_140ms_ease-out_both]"
 >

 {/* Result count header */}
 {query && (
 <div className="border-slate-100 px-3 py-2">
 ...
 <p className="text-slate-500">No skills match &ldquo;{query}&rdquo;</p>
 </li>
 )}
 </ul>

 {/* Footer hint */}
 <div className="border-slate-100 px-3 py-2 flex items-center gap-3">
 <kbd className="rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-slate-500">↑↓</kbd>
 <span className="text-slate-400">navigate</span>
 <kbd className="rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-slate-500">↵</kbd>
 <span className="text-slate-400">select</span>
 <kbd className="rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-slate-500">⌫</kbd>
 <span className="text-slate-400">remove last</span>
 <kbd className="ml-auto rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-slate-500">Esc</kbd>
 ...
 {/* Max selection hint */}
 {reachedMax && (
 <p role="alert" className="mt-1.5 text-amber-600">
