...
export async function generateCoverLetter(input: CoverLetterInput): Promise<string> {
  ...
  const projectSnippet = studentProjects
    .slice(0, 3)
...
`;
  ...
  try {
      ...
      [
        {
          role: 'system',
          content:
            'You are an expert career coach helping students write compelling, authentic cover letters for freelance gig applications.',
        },
    ...
    console.warn('[coverLetter] AI generation error:', error);
...
export async function improveCoverLetter(
  ...
  try {
      ...
      [
        {
          role: 'system',
          content:
            'You are a career coach. Improve the student cover letter based on the feedback provided. Keep the same voice and length.',
        },
        {
          role: 'user',
    ...
    console.warn('[coverLetter] AI improve error:', error);
